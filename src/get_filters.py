...
class GetFilters:
    ...
    def get_filtre(
        ...
        """
        logging.info(
            f"Starting get_filtre for brand={brand}, model={model}, color={color}"
        )
    ...
    def build_filtre(
        ...
        """
        logging.info(
            f"Building filter: brand={brand}, model={model}, color={color}, type={vehicle_type}"
        )
    ...
    def handle_models(self, brand: str, models: dict) -> None:
        ...
        for model, total_number in models.items():
            ...
            if total_number < 3000:
                ...
                logging.info(
                    f"Model {model} exceeds 3000 ads, drilling down to colors"
                )
    ...
    def handle_types(
        ...
        for vehicle_type, total_number in types.items():
            if total_number < 3000:
                ...
                logging.info(
                    f"Type {vehicle_type} exceeds 3000 ads, drilling down to fuels"
                )
    ...
    def handle_fuels(self, brand, model, color, vehicle_type, fuels) -> None:
        ...
        for fuel, total_number in fuels.items():
            if total_number < 3000:
                ...
                logging.info(
                    f"Fuel {fuel} exceeds 3000 ads, drilling down to gearboxes"
                )
    ...
    def handle_gearboxes(
        self, brand, model, color, vehicle_type, fuel, gearboxes
    ) -> None:
        ...
        for gearbox, total_number in gearboxes.items():
            if total_number < 3000:
                ...
                logging.info(
                    f"Gearbox {gearbox} exceeds 3000 ads, drilling down to owner types"
                )
    ...
    def handle_owner_types(
        ...
        for owner_type, total_number in owner_types.items():
            if total_number < 3000:
                logging.info(
                    f"Adding filter for owner type {owner_type} (<3000 ads)"
                )
                self.filters.append(
                    self.build_filtre(
                        brand,
                ...
                logging.info(
                    f"Owner type {owner_type} exceeds 3000 ads, adding to over_3000_filters"
                )
    ...
    def run(self, brand: str) -> None:
        ...
        logging.info(
            "Filters exceeding 3000 ads: %d", len(self.over_3000_filters)
        )
        with open("filters.json", "w") as f:
            json_str = json.dumps(self.filters)
            f.write(json_str)
